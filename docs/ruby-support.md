...
- **`build-aux/modules/ruby-gnome.json`** — the full 20-gem ruby-gnome closure
  at 4.3.8, rooted at `adwaita` + `gtk4` + `gobject-introspection`. Flatpak
...
Verified so far, without a Flatpak build: all 21 pinned checksums (20 gems +
CRuby + libyaml) match the real downloads, and `gem install --local --explain`
against a clean `GEM_HOME` with the network blocked resolves the whole set in
the declared order — i.e. the closure is complete and offline-installable.
**Still unverified:** the native extension builds (`glib2`, `cairo`,
`gobject-introspection`, `gio2`, `gdk_pixbuf2`, `pango`, `cairo-gobject`) against
the GNOME 50 SDK's headers, and whether ruby-gnome 4.3.8 is happy on Ruby 4.0 —
both need an actual `flatpak-builder` run.
